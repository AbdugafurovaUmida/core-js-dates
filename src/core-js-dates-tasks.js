...
function getTime(date) {
  ...
  return dates.toTimeString().split(' ')[0];
}
...
function getDayName(date) {
  ...
  const result = dateObj.toLocaleString('en-US', {
    timeZone: 'UTC',
    weekday: 'long',
  });
  return result;
...
function getNextFriday(date) {
  ...
  const nextNumberFriday = day < 5 ? 5 - day : 5 + (7 - day);
  const nextFriday = new Date(date);
  nextFriday.setDate(dateObj.getDate() + nextNumberFriday);
  return nextFriday;
}
...
function formatDate(date) {
  ...
  return `${dateObj.getMonth() + 1}/${dateObj.getDate()}/${dateObj.getFullYear()}, ${dateObj.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', second: '2-digit' })}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const msPerCurrentDate = Date.parse(dates);
